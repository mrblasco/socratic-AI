...
tutorgpt %>%
  ...
  dplyr::count(name, value, treatment_tutor_id) %>%
  na.omit() %>%
  ...
  tidyr::pivot_wider(names_from = treatment_tutor_id,
                     ...
                     values_fill = 0) %>%
  dplyr::mutate(name = ifelse(row_number() == 1, name, ""), .by = name) %>% 
  dplyr::mutate(totals = n_non_socratic + n_socratic) %>%
  dplyr::select(
    "Name" = name,
    "Value" = value,
    "% Non-Socratic" = percent_non_socratic,
    "% Socratic" = percent_socratic,
    "N" = totals
  ) %>% 
  kableExtra::kbl(
    digits = 1,
