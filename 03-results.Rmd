...
table_socratic <- tutorgpt_rename %>%
    dplyr::select(-treatment_explain_id) %>%
    tidyr::pivot_longer(-treatment_tutor_id) %>%
    dplyr::mutate(
        pval = xtabs(~ value + treatment_tutor_id) %>%
            fisher.test() %>%
            .$p.value,
        .by = name
    ) %>%
...
table_explain <- tutorgpt_rename %>%
    dplyr::select(-treatment_tutor_id) %>%
    tidyr::pivot_longer(-treatment_explain_id) %>%
    dplyr::mutate(
        pval_explain = xtabs(~ value + treatment_explain_id) %>%
            fisher.test() %>%
            .$p.value,
        .by = name,
    ) %>%
    ...
    tidyr::pivot_wider(
        names_from = treatment_explain_id,
        values_from = c(percent, n),
        values_fill = 0
    )

table_socratic %>%
    dplyr::left_join(table_explain, by = c("name", "value")) %>%
    dplyr::mutate(
        name = ifelse(row_number() == 1, name, ""),
    ...
    kableExtra::add_header_above(c(" " = 2, "Socratic (%)" = 3, "AI Explain (%)" = 3, " "))
```
...
attitudes_df %>%
    ...
    geom_text(
        aes(hjust = ifelse(percent < 5, -0.5, 0.5)),
...
```{r update-propensity, include = FALSE}
update_ds <- tutorgpt %>%
    dplyr::mutate(
        update = sign(q12_guess_coins_final - q9_guess_coins_initial),
        initial_guess_100 = q9_guess_coins_initial / 100
    )

update_propensity_table <- update_ds %>%
    xtabs(formula = ~ update + treatment_explain_id)

knitr::kable(update_propensity_table)
fisher.test(update_propensity_table)

model <- center(q12_guess_coins_final) ~ initial_guess_100 +
    I(initial_guess_100^2) + treatment_explain_id + location

fit_update_propensity <- lm(model, update_ds)
fit_robust <- rlm(model, update_ds)
fit_no_out <- fit_robust %>%
    update(subset = q9_guess_coins_initial < 1e5)

fit <- list(fit_update_propensity, fit_robust, fit_no_out)
stargazer::stargazer(fit, type = "text", keep.stat = "n", digits = 1)

# ---- Figure ----
cap <- "Difference between first and second guess.  "
tutorgpt %>%
    dplyr::mutate(
        final_tc = q12_guess_coins_final %>%
            topcode(1e3),
        initial_tc = q9_guess_coins_initial %>%
            topcode(1e3),
    ) %>%
    ggplot(
        aes(
            x = initial_tc,
            y = final_tc,
            color = treatment_explain_id,
            pch = treatment_explain_id,
            fill = treatment_explain_id,
            linetype = treatment_explain_id
        )
    ) +
    geom_point() +
...
bootstrap_accuracy <- tutorgpt %>%
    dplyr::filter(q12_guess_coins_final < 6e3) %>%
    dplyr::mutate(abs_err = abs(q12_guess_coins_final - coins_value)) %>%
    dplyr::reframe(
...
bootstrap_accuracy_diff <- bootstrap_accuracy %>%
    dplyr::summarise(diff = diff(boot_med), .by = sim_id)


alpha <- 0.1
diff_cis <- bootstrap_accuracy_diff %>%
    dplyr::reframe(quantile(diff, p = c(alpha / 2, 1 - alpha / 2)))
...
```{r credibility-ai, include = FALSE, tab.cap = cap}
cap <- paste(
    "Percentage of students who perceived the accuracy of the AI-generated",
    "estimate for the coin value in the jar as accurate"
)
...
wilcox.test(as.numeric(q10_guess_coins_ai_accuracy) ~ treatment_explain_id, tutorgpt)
```

```{r credibility-human, include = FALSE, tab.cap = cap}
cap <- paste(
    "% of students who perceived the accuracy of the Human-generated average",
    "guess for the coin value in the jar as _____"
)
xtab <- xtabs(~ q11_guess_coins_human_accuracy + treatment_explain_id, tutorgpt)
kable(prop.table(xtab, margin = 2) * 100, digits = 0)

wilcox.test(as.numeric(q11_guess_coins_human_accuracy) ~ treatment_explain_id, tutorgpt)
```

Conversely, exposure to the AI step-by-step reasoning seemed associated with students' perception of the accuracy of the "human" guess --- the average guess from 600 participants reported in the original study [@steiner2015turns].^[
...
] While all groups received the same human and AI estimates, 64% of students in the control rated the human estimate as "poor" or "very poor" against 45% of students exposed to AI reasoning. 

Statistically, a Mann-Whitney U test indicates a significant difference between the two conditions at the 10% level ($W = 2179$, $p = .087$). Likewise, regression analysis that controls for fixed differences across schools and across individuals (including gender, the initial guess of the value of coins, and experience with ChatGPT) showed a significant effect (p < 0.05) of the assignment to AI reasoning on students' perceptions of the human guess, as shown in Figure \@ref(fig:perceived-accuracy-coins). These findings underscore the complex relationship between AI and learning. It suggests that students exposed to the AI's step-by-step reasoning may have identified minor errors, thus increasing their expectations of human accuracy, and those who viewed AI as a "black box" tended to underrate human estimates.
...
list("Human" = fit, "AI" = fit_ai) %>%
    ...
    scale_y_discrete(
        labels = c(
            Human = "Perceived human accuracy",
...
```{r messages, fig.cap = "Comparison of message frequency and word count per message across Socratic and non-Socratic treatments. Top panel (**A**) shows differences in the frequency of messages exchanged, while the bottom panels (**B** ) depict the word counts per message for the AI agent (left) and the students (right).", fig.width = 7, fig.height = 7, echo = FALSE}
palette <- c("#d95f02", "#7570b3")
...
p_combined <-
    (p_messages + labs(
        ...
        subtitle = NULL,
        caption = NULL,
        x = "Messages Exchanged with AI Agent (per student)",
        y = NULL,
    )) /
        (p_words + labs(
            title = NULL,
...
ds_text <- read.csv(data_path, header = FALSE, col.names = c(
...
ds <- left_join(ds_text, chat_info, by = "chat_id") %>%
...
```{r socratic-dimensions, fig.cap = cap}
cap <- paste(
    "Coefficients and 95% confidence intervals for the effect of Socratic AI from mixed-effects regression models. Each row reports the estimated average difference between Socratic and non-Socratic AI for a distinct pedagogical dimension. All models include individual-level random effects to account for within-student correlation across chats, school-location fixed effects, and a control for the number of exchanges in each chat. Dependent variables were standardized from their original 1–5 scales, so all coefficients represent changes measured in standard-deviation units." # nolint
)


coeffs <- models_socratic %>%
    lapply(broom::tidy, conf.int = TRUE) %>%
    bind_rows(.id = "depvar")

coeffs %>%
    filter(grepl("socratic", term)) %>%
    ggplot() +
    aes(
        x = estimate,
        y = reorder(depvar, estimate),
        xmin = conf.low, xmax = conf.high,
        color = "95% CI",
    ) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    geom_coef() +
    labs(
        x = "Regression coefficient (Socratic AI)",
        y = "Pedagogical dimension (standardised)",
        caption = "Source: AI-student conversations (n = 362)"
    ) +
    theme(
...
Further explorative regression analysis to examine potential treatment interactions shows that Socratic AI students with prior ChatGPT experience reported significantly less confidence (p < 0.1) than Non-Socratic AI students (see Figure \@ref(fig:interactions)). This explorative finding suggests that experienced users may perceive new or unconventional AI chatbots as less effective or even counterproductive, indicating that encouraging the use of such AI chatbots among experienced ChatGPT students can be challenging.

### Perceived Helpfulness of AI Agent

To test differences in students' perceptions of the AI agent's helpfulness (RQ5), we examined students' ratings of helpfulness of the AI interaction at the end of each task. These ratings were on a five-point scale, from "Not at all helpful" to "Very helpful."[^mistake]
...
```{r helpfulness, fig.width = 7, fig.height = 7, fig.cap = "This figure shows the impact of Socratic AI on students' perceived helpfulness of the AI agent across three different tasks."}
...
df_coef %>%
    ...
    labs(
        x = "Regression coefficient (ordinal logistic)",
        y = "Sample",
        title = "Effect of Socratic AI on Students' Perceived Helpfulness of the AI Agent",
    )
```
...
panel_a <- tutorgpt %>%
    ...
    reframe(
        broom::tidy(lm(value == "correct" ~ treatment_tutor_id), conf.int = TRUE),
        .by = name
    ) %>%
    mutate(
        name = case_match(
    ...
    labs(
        ...
        subtitle = "Comparison of Correct Answers Before and After Interacting with AI Agent"
    )
...
```{r interactions, include = TRUE, fig.cap = cap, fig.asp = 1}
cap <- "Regression coefficients and 95% confidence intervals from a linear regression using as dependent variable the (**A**) students' self-reported confidence level and (**B**) perceived helpfulness of the AI agent. The controls include student's gender, grades, location, and ChatGPT experience. Treatment dummies are interacted with all the controls and the task type. All models also include individual student random effects, and the student's rated confidence in their skills before performing each task." # nolint

tutorgpt_wide <- tutorgpt_wide %>%
    mutate(
        helpful_yes = helpful %in% c("Helpful", "Very helpful"),
        confident_yes = value %in% c("Very confident", "Confident")
    )

fit_lmer <- lmerTest::lmer(
    100 * confident_yes ~ treatment_tutor_id + name +
        conf_value + location + gender + has_used_before +
        top_grades + (1 | student_id), tutorgpt_wide
)

interactions <- list(baseline = fit_lmer)
interactions$used <- update(fit_lmer, ~ . + treatment_tutor_id:has_used_before)
interactions$name <- update(fit_lmer, ~ . + treatment_tutor_id:name)
interactions$gender <- update(fit_lmer, ~ . + treatment_tutor_id:gender)
interactions$grades <- update(fit_lmer, ~ . + treatment_tutor_id:top_grades)
interactions$all <- update(fit_lmer, ~ . + treatment_tutor_id:(has_used_before + top_grades + gender + location + name))

coef_df <- interactions %>%
    lapply(broom.mixed::tidy, conf.int = TRUE) %>%
    bind_rows(.id = "model")

calc_stars <- function(p.value) {
    dplyr::case_when(
        p.value < 0.01 ~ "***",
...
panel_a <- coef_df %>%
    filter(grepl(":", term), model == "all") %>%
    ggplot() +
    aes(x = estimate, y = term, xmin = conf.low, xmax = conf.high, color = p.value < 0.1) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    geom_linerange(linewidth = 3, alpha = .25) +
    geom_point(size = 3) +
    geom_text_repel(
        nudge_y = -.25,
        aes(
            label = sprintf("%.1f%s", estimate, calc_stars(p.value))
        )
    ) +
    ## Add CIs
    geom_text(x = Inf, aes(label = sprintf("(%s: %.1f, %.1f)", "95% CI", conf.low, conf.high)), size = 3.5) +
    coord_cartesian(clip = "off") +
    theme(
        legend.position = "none",
        axis.text.y = element_markdown(hjust = 0),
        plot.margin = margin(t = .1, b = .1, r = 1, l = .1, unit = "in"),
    ) +
    ...
    scale_y_discrete(
        labels = function(x) {
            x %>%
                gsub("^.*:", "Socratic x ", .) %>%
                gsub("has_used_beforeYes", "ChatGPT experience", .) %>%
                gsub("locationSeville", "Seville", .) %>%
                gsub("top_grades", "", .) %>%
                gsub("gender", "", .) %>%
                gsub("nametask([123])_(.*)", "Task \\1 (\\2)", .)
        }
    ) +
    labs(
        title = "Socratic AI and Student Confidence Levels",
        subtitle = "Interaction effects between Socratic AI and task/student characteristics",
        y = NULL,
        x = "Regression coefficients and 95% confidence intervals",
        caption = "Results from linear regression (n = 366); * p < 0.1"
    )

# ggsave("figures_and_tables/10_socratic_interact_confidence.pdf")

#' ### Helpfulness

fit_lmer <- lmerTest::lmer(
    100 * helpful_yes ~ treatment_tutor_id + name +
        conf_value + location + gender + has_used_before +
        top_grades + (1 | student_id), tutorgpt_wide
)
...
panel_b <- coef_df %>%
    ...
    geom_text_repel(
        nudge_y = -.25,
        aes(label = sprintf("%.1f%s", estimate, calc_stars(p.value)))
    ) +
    ## Add CIs
    geom_text(x = Inf, aes(label = sprintf("(%s: %.1f, %.1f)", "95% CI", conf.low, conf.high)), size = 3.5) +
    coord_cartesian(clip = "off") +
    ...
    labs(
        title = "Socratic AI and Perceived Helpfulness",
        subtitle = "Interaction effects between Socratic AI and task/student characteristics",
        y = NULL,
        x = "Regression coefficients and 95% confidence intervals",
        caption = "Results from linear regression (n = 366); * p < 0.1"
...
# ggsave("figures_and_tables/10_socratic_interact_helpfull.pdf")

panel_a / panel_b + plot_annotation(tag_levels = "A")

## ggsave("figures_and_tables/10_socratic_interactions.pdf")
```